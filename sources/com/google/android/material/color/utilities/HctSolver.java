...
public class HctSolver {
    ...
    static boolean areInCyclicOrder(double d6, double d7, double d8) {
        return sanitizeRadians(d7 - d6) < sanitizeRadians(d8 - d6);
    }

    static double[] bisectToLimit(double d6, double d7) {
        int criticalPlaneAbove;
        int criticalPlaneBelow;
        double[][] bisectToSegment = bisectToSegment(d6, d7);
        ...
        for (int i5 = 0; i5 < 3; i5++) {
            ...
            if (d8 != d9) {
                if (d8 < d9) {
                    criticalPlaneAbove = criticalPlaneBelow(trueDelinearized(d8));
                    criticalPlaneBelow = criticalPlaneAbove(trueDelinearized(dArr2[i5]));
                } else {
                    criticalPlaneAbove = criticalPlaneAbove(trueDelinearized(d8));
                    criticalPlaneBelow = criticalPlaneBelow(trueDelinearized(dArr2[i5]));
                }
                int i6 = criticalPlaneAbove;
                int i7 = criticalPlaneBelow;
                for (int i8 = 0; i8 < 8 && Math.abs(i7 - i6) > 1; i8++) {
                    double d10 = i6 + i7;
                    Double.isNaN(d10);
                    int floor = (int) Math.floor(d10 / 2.0d);
                    double[] coordinate = setCoordinate(dArr, CRITICAL_PLANES[floor], dArr2, i5);
                    double hueOf2 = hueOf(coordinate);
                    if (areInCyclicOrder(hueOf, d7, hueOf2)) {
                        dArr2 = coordinate;
                        i7 = floor;
                    } else {
                        dArr = coordinate;
    ...
    static double[][] bisectToSegment(double d6, double d7) {
        ...
        for (int i5 = 0; i5 < 12; i5++) {
            double[] nthVertex = nthVertex(d6, i5);
            if (nthVertex[0] >= 0.0d) {
                ...
                if (!z5) {
                    ...
                    d9 = d8;
                    z5 = true;
                } else if (z6 || areInCyclicOrder(d8, hueOf, d9)) {
                    if (areInCyclicOrder(d8, d7, hueOf)) {
                        dArr2 = nthVertex;
                        d9 = hueOf;
                    } else {
                        dArr = nthVertex;
    ...
    static double chromaticAdaptation(double d6) {
        double pow = Math.pow(Math.abs(d6), 0.42d);
        double signum = MathUtils.signum(d6);
        Double.isNaN(signum);
        return ((signum * 400.0d) * pow) / (pow + 27.13d);
    }

    static int criticalPlaneAbove(double d6) {
        return (int) Math.ceil(d6 - 0.5d);
    }

    static int criticalPlaneBelow(double d6) {
        return (int) Math.floor(d6 - 0.5d);
    }

    static int findResultByJ(double d6, double d7, double d8) {
        double sqrt = Math.sqrt(d8) * 11.0d;
        ViewingConditions viewingConditions = ViewingConditions.DEFAULT;
        double pow = 1.0d / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d);
        double cos = (Math.cos(d6 + 2.0d) + 3.8d) * 0.25d * 3846.153846153846d * viewingConditions.getNc() * viewingConditions.getNcb();
        ...
        while (i5 < 5) {
            ...
            double d10 = pow;
            double pow2 = Math.pow(((d7 == 0.0d || sqrt == 0.0d) ? 0.0d : d7 / Math.sqrt(d9)) * pow, 1.1111111111111112d);
            double d11 = sqrt;
            double aw = (viewingConditions.getAw() * Math.pow(d9, (1.0d / viewingConditions.getC()) / viewingConditions.getZ())) / viewingConditions.getNbb();
            double d12 = (((0.305d + aw) * 23.0d) * pow2) / (((23.0d * cos) + ((pow2 * 11.0d) * cos2)) + ((pow2 * 108.0d) * sin));
            double d13 = d12 * cos2;
            double d14 = d12 * sin;
            double d15 = aw * 460.0d;
            double[] matrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation((((451.0d * d13) + d15) + (288.0d * d14)) / 1403.0d), inverseChromaticAdaptation(((d15 - (891.0d * d13)) - (261.0d * d14)) / 1403.0d), inverseChromaticAdaptation(((d15 - (d13 * 220.0d)) - (d14 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            double d16 = matrixMultiply[0];
            if (d16 < 0.0d) {
                break;
            }
            double d17 = matrixMultiply[1];
            if (d17 < 0.0d) {
                break;
            }
            double d18 = matrixMultiply[2];
            if (d18 < 0.0d) {
                break;
            }
    ...
    static double intercept(double d6, double d7, double d8) {
        return (d7 - d6) / (d8 - d6);
    }

    static double inverseChromaticAdaptation(double d6) {
        double abs = Math.abs(d6);
        double max = Math.max(0.0d, (27.13d * abs) / (400.0d - abs));
        double signum = MathUtils.signum(d6);
        double pow = Math.pow(max, 2.380952380952381d);
        Double.isNaN(signum);
        return signum * pow;
    }

    static boolean isBounded(double d6) {
        return 0.0d <= d6 && d6 <= 100.0d;
    }

    static double[] lerpPoint(double[] dArr, double d6, double[] dArr2) {
        double d7 = dArr[0];
        double d8 = dArr[1];
        double d9 = dArr[2];
        return new double[]{d7 + ((dArr2[0] - d7) * d6), d8 + ((dArr2[1] - d8) * d6), d9 + ((dArr2[2] - d9) * d6)};
    }
    ...
    static double[] nthVertex(double d6, int i5) {
        double[] dArr = Y_FROM_LINRGB;
        double d7 = dArr[0];
        double d8 = dArr[1];
        double d9 = dArr[2];
        double d10 = i5 % 4 <= 1 ? 0.0d : 100.0d;
        double d11 = i5 % 2 != 0 ? 100.0d : 0.0d;
        if (i5 < 4) {
            double d12 = ((d6 - (d8 * d10)) - (d9 * d11)) / d7;
            return isBounded(d12) ? new double[]{d12, d10, d11} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        if (i5 < 8) {
            double d13 = ((d6 - (d7 * d11)) - (d9 * d10)) / d8;
            return isBounded(d13) ? new double[]{d11, d13, d10} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        double d14 = ((d6 - (d7 * d10)) - (d8 * d11)) / d9;
        return isBounded(d14) ? new double[]{d10, d11, d14} : new double[]{-1.0d, -1.0d, -1.0d};
    }

    static double sanitizeRadians(double d6) {
        return (d6 + 25.132741228718345d) % 6.283185307179586d;
    }

    static double[] setCoordinate(double[] dArr, double d6, double[] dArr2, int i5) {
        return lerpPoint(dArr, intercept(dArr[i5], d6, dArr2[i5]), dArr2);
    }

    public static Cam16 solveToCam(double d6, double d7, double d8) {
        return Cam16.fromInt(solveToInt(d6, d7, d8));
    }

    public static int solveToInt(double d6, double d7, double d8) {
        if (d7 < 1.0E-4d || d8 < 1.0E-4d || d8 > 99.9999d) {
            return ColorUtils.argbFromLstar(d8);
        }
        double sanitizeDegreesDouble = (MathUtils.sanitizeDegreesDouble(d6) / 180.0d) * 3.141592653589793d;
        double yFromLstar = ColorUtils.yFromLstar(d8);
        int findResultByJ = findResultByJ(sanitizeDegreesDouble, d7, yFromLstar);
        return findResultByJ != 0 ? findResultByJ : ColorUtils.argbFromLinrgb(bisectToLimit(yFromLstar, sanitizeDegreesDouble));
    }

    static double trueDelinearized(double d6) {
        double d7 = d6 / 100.0d;
        return (d7 <= 0.0031308d ? d7 * 12.92d : (Math.pow(d7, 0.4166666666666667d) * 1.055d) - 0.055d) * 255.0d;
    }
}
