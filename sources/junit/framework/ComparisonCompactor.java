...
public class ComparisonCompactor {
    ...
    private String computeCommonPrefix() {
        return (this.fPrefix > this.fContextLength ? ELLIPSIS : BuildConfig.FLAVOR) + this.fExpected.substring(Math.max(0, this.fPrefix - this.fContextLength), this.fPrefix);
    }

    private String computeCommonSuffix() {
        int end = Math.min((this.fExpected.length() - this.fSuffix) + 1 + this.fContextLength, this.fExpected.length());
        return this.fExpected.substring((this.fExpected.length() - this.fSuffix) + 1, end) + ((this.fExpected.length() - this.fSuffix) + 1 < this.fExpected.length() - this.fContextLength ? ELLIPSIS : BuildConfig.FLAVOR);
    }
