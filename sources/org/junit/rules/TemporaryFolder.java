...
public class TemporaryFolder extends ExternalResource {
    ...
    public File newFolder(String... folderNames) throws IOException {
        File file = getRoot();
        int i5 = 0;
        while (i5 < folderNames.length) {
            String folderName = folderNames[i5];
            File file2 = new File(file, folderName);
            if (file2.mkdir() || !isLastElementInArray(i5, folderNames)) {
                i5++;
                file = file2;
            } else {
    ...
    private File createTemporaryFolderIn(File parentFolder) throws IOException {
        File createdFolder = File.createTempFile("junit", BuildConfig.FLAVOR, parentFolder);
